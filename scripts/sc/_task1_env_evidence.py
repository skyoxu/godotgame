...
def _resolve_solution_file(root: Path) -> Path | None:
    all_solutions = sorted(root.glob("*.sln"))
    if not all_solutions:
        return None
    if len(all_solutions) == 1:
        return all_solutions[0]

    preferred_keywords = ("game", "godot")
...
def _resolve_task1_checklist(root: Path) -> Path | None:
    checklist_env = str(os.environ.get("TASK1_ACCEPTANCE_CHECKLIST") or "").strip()
    if checklist_env:
        candidate = (root / checklist_env).resolve() if not Path(checklist_env).is_absolute() else Path(checklist_env)
        if candidate.is_file():
            return candidate

    fallback = sorted((root / "docs" / "architecture" / "overlays").glob("**/08/ACCEPTANCE_CHECKLIST.md"))
...
def step_task1_env_evidence(out_dir: Path, *, godot_bin: str | None) -> StepResult:
    root = repo_root()
    date = today_str()
    expected_godot_version = str(os.environ.get("GODOT_VERSION") or "4.5.1").strip()
    ...
    solution_file = _resolve_solution_file(root)
    restore_command = ""
    if solution_file is not None:
        rel_solution = f".\\{solution_file.name}"
        restore_command = f"dotnet restore {rel_solution}"
        _run_command(["dotnet", "restore", rel_solution], cwd=root, timeout_sec=240)
        rc_restore, out_restore = _run_command(["dotnet", "restore", rel_solution], cwd=root, timeout_sec=240)
    else:
        rc_restore = 0
        out_restore = "skipped: no .sln file found in repository root\n"
        details["errors"].append("warning:no_solution_file_found")
    _write_utf8_file(evidence_dir / "dotnet-restore.txt", out_restore)
    details["commands"]["dotnet_restore_command"] = {
        "rc": rc_restore,
        "solution": solution_file.name if solution_file is not None else "",
    }
    ...
    task_payload: dict[str, Any] = {
        ...
        "godot_commands": {
            "godot_version_command": {
                ...
                "parsed_version": expected_godot_version if _contains_token(out_godot_path, expected_godot_version) else "",
            ...
            "godot_bin_version_command": {
                ...
                "parsed_version": expected_godot_version if _contains_token(out_godot_bin, expected_godot_version) else "",
    ...
    checklist_path = _resolve_task1_checklist(root)
    ...
    if checklist_path is not None:
        utf8_checked_files.insert(1, _rel(root, checklist_path))
    else:
        details["errors"].append("warning:no_overlay_acceptance_checklist_found")
    ...
    details["checks"]["godot_path_version_ok"] = rc_godot_path == 0 and _contains_token(out_godot_path, expected_godot_version)
    details["checks"]["godot_bin_version_ok"] = rc_godot_bin == 0 and _contains_token(out_godot_bin, expected_godot_version)
    ...
    details["checks"]["task1_checklist_found"] = checklist_path is not None
    details["checks"]["solution_file_found"] = solution_file is not None
