...
def step_task_test_refs_validate(out_dir: Path, triplet: TaskmasterTriplet, *, require_non_empty: bool) -> StepResult:
    ...
    return run_and_capture(out_dir, name="validate-task-test-refs", cmd=cmd, timeout_sec=60)
...
def step_security_soft(out_dir: Path) -> StepResult:
    ...
    if domain_check_env and not domain_check.is_absolute():
        domain_check = root / domain_check_env
    if domain_check.is_file():
        rel = str(domain_check.relative_to(root)).replace("\\", "/")
        steps.append(run_and_capture(out_dir, "check-domain-contracts", ["py", "-3", rel], 60))
    ...
    encoding_check = root / "scripts" / "python" / "check_encoding.py"
    if encoding_check.is_file():
