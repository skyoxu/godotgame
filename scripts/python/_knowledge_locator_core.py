"""Deterministic, location-only retrieval for template repository knowledge."""
...
TASK_VIEW_ID = re.compile(r"\b[A-Z]{2,8}-\d+\b", re.IGNORECASE)
...
def tokens(query: str) -> list[str]:
    ...
    for part in parts:
        ...
        if re.fullmatch(r"[\u3400-\u9fff]+", part) and len(part) > 2:
            values += [part[index:index + 2] for index in range(len(part) - 1)]
        for value in values:
...
def _explicit(query: str, module: dict[str, Any]) -> bool:
    ...
    values = {
        ...
        str(module.get("module_id", "")).casefold(),
        str(module.get("title", "")).casefold(),
    }
...
def _task_identity_line(module: dict[str, Any], query: str, lines: list[str]) -> int | None:
    ...
    for view_id in view_ids:
        pattern = re.compile(rf'^\s*"(?:id|task_id)"\s*:\s*"{re.escape(view_id)}"\s*,?\s*$', re.IGNORECASE)
    ...
    for task_number in task_numbers:
        pattern = re.compile(rf'^\s*"(?:id|taskmaster_id)"\s*:\s*{re.escape(task_number)}\s*,?\s*$')
        for index, line in enumerate(lines, 1):
            if pattern.match(line):
                return index
    return None
...
def locate(request: dict[str, Any], catalog: dict[str, Any], policy: dict[str, Any], eligible_ids: set[str], max_candidates: int) -> dict[str, Any]:
    ...
    for module in by_id.values():
        ...
        score = matches * 10 + path_token_matches * 8 + title_token_matches * 8
        if phrase:
            score += 25
        if exact:
            score += 100
        ...
        boosted = policy_exact_path and entrypoint_token_matches > 0
        if boosted:
        ...
        ranked[str(module_id)] = (score, source_path.casefold(), {
            ...
            "score": score,
            "snippet": "\n".join(str(module.get("content", "")).splitlines()[max(0, line_start - 2):line_start + 2])[:1200],
            ...
            "rank_evidence": {
                ...
                "policy_exact_path_bonus": POLICY_EXACT_PATH_BONUS if boosted else 0,
    ...
    for score, module in base:
        for relation in module.get("relations", []):
            ...
            if not _policy_allows(related, query, policy):
                continue
            anchor, line_start, line_end, strategy = _best_location(related, query, qtokens)
            ...
            ranked[related["module_id"]] = (relation_score, related["source_path"].casefold(), {
                ...
                "line_start": line_start, "line_end": line_end, "line": line_start,
                "source_sha256": related["source_sha256"], "primary_domain": related["primary_domain"],
                "status": related["status"], "score": relation_score,
                "snippet": "", "provenance": ["catalog-v1", catalog["source_snapshot"]["ref"]],
                "rank_evidence": {"strategy": "relation-expansion", "score": relation_score, "token_matches": 0,
                    "confidence": "medium", "task_identity_match": strategy == "task-identity", "task_identity_bonus": 0,
