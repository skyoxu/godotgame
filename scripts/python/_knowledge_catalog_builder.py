"""Deterministic, template-safe repository knowledge catalog builder."""
...
ROOT_EXACT_SOURCES = {
    ...
    "docs/PROJECT_DOCUMENTATION_INDEX.md", "docs/testing-framework.md", ".taskmaster/docs/prd.txt",
}
SOURCE_PREFIXES = (
    "docs/knowledge/", ".agents/skills/", "docs/agents/", "docs/prd/", "docs/gdd/",
    "docs/game-type-guides/", "docs/adr/", "docs/architecture/", "docs/workflows/",
    "Game.Core/Contracts/", ".taskmaster/tasks/", "execution-plans/", "decision-logs/",
)
...
class GitSnapshot:
    def __init__(self, root: Path, authority_ref: str = "refs/heads/main") -> None:
        ...
        self.commit = subprocess.check_output(["git", "-C", str(self.root), "rev-parse", "--verify", authority_ref], text=True, encoding="utf-8").strip()
        if not re.fullmatch(r"[0-9a-f]{40}", self.commit):
            raise ValueError("invalid_authority_commit")
        raw = subprocess.check_output(["git", "-C", str(self.root), "ls-tree", "-r", "--name-only", self.commit], text=True, encoding="utf-8")
        self.paths = tuple(sorted(normalize_path(path) for path in raw.splitlines() if path.strip()))
...
def _excluded(path: str, exclusions: dict[str, Any]) -> bool:
    for rule in exclusions.get("rules", []):
        if not isinstance(rule, dict):
            continue
        prefix = rule.get("path_prefix", rule.get("prefix"))
...
def _eligible_source(path: str) -> bool:
    return path in ROOT_EXACT_SOURCES or (path.startswith(SOURCE_PREFIXES) and PurePosixPath(path).suffix.casefold() in TEXT_SUFFIXES)
...
def _anchors(content: str, fallback: str) -> list[dict[str, Any]]:
    ...
    headings = [(i, len(m.group(1)), m.group(2).strip()) for i, line in enumerate(lines, 1) if (m := HEADING.match(line))]
    if not headings:
...
def _status(path: str, content: str) -> tuple[str, bool]:
    if path.startswith("docs/adr/") and "/addenda/" not in path:
        ...
        return ({"accepted": "active", "proposed": "conditional", "superseded": "historical"}.get(raw, "excluded"), raw in {"accepted", "proposed", "superseded"})
...
def _classification(path: str) -> tuple[str, tuple[str, ...], str, str]:
    if path in {"AGENTS.md", "workflow.md", "DELIVERY_PROFILE.md", "docs/testing-framework.md", "docs/PROJECT_DOCUMENTATION_INDEX.md"} or path.startswith((".agents/skills/", "docs/agents/", "docs/workflows/")):
...
def _consumer_ids(primary: str) -> list[str]:
    return ["repository-session", "chapter5", "chapter6", "review"] if primary == "delivery" else ["repository-session", "chapter4", "chapter5", "chapter6", "review"]
...
def build_layers(snapshot: GitSnapshot, exclusions: dict[str, Any], policies: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    for path in snapshot.paths:
        ...
        modules.append({
            "module_id": module_id, "kind": kind, "title": title, "primary_domain": primary,
            "visibility": _visibility(primary, deps), "lifecycle": "repository-source", "enforcement_level": "E1",
            "authority_class": "derived-cache", "source_role": role, "status": status,
            "semantic_eligible": semantic_eligible, "source_path": path, "source_sha256": digest,
            "anchor": title, "anchors": _anchors(content, title), "consumer_ids": _consumer_ids(primary),
    ...
    sources.sort(key=lambda item: item["path"]); modules.sort(key=lambda item: item["module_id"])
    snapshot_doc = {
        "schema": "godot-project-knowledge.repository-source-snapshot.v1",
        "snapshot_id": sha256_bytes((snapshot.authority_ref + "\0" + snapshot.commit + "\0" + json.dumps(sources, sort_keys=True)).encode("utf-8")),
        "ref": snapshot.authority_ref, "commit": snapshot.commit, "sources": sources,
    }
    catalog = {"schema": "godot-project-knowledge.repository-catalog.v1", "source_snapshot": snapshot_doc, "domains": list(DOMAINS), "modules": modules}
    projection_items = []
    for policy in policies.get("policies", []):
        ...
        for module in modules:
            ...
            if consumer not in module.get("consumer_ids", []) or module.get("status") not in allowed_statuses or not module.get("semantic_eligible", True):
                continue
            if not any(module.get("visibility", {}).get(domain) in allowed_visibility for domain in allowed_domains):
                continue
    ...
    projections = {
        "schema": "godot-project-knowledge.consumer-projections.v1", "source_snapshot_id": snapshot_doc["snapshot_id"],
        "catalog_sha256": prefixed_sha256(canonical_bytes(catalog)), "policy_revision": policies["policy_revision"],
        "policy_sha256": prefixed_sha256(canonical_bytes(policies)), "projections": projection_items,
    }
