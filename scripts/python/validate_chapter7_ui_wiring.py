...
def validate(*, repo_root: Path) -> tuple[int, dict[str, Any]]:
    ...
    if summary.get("status") == "skipped":
        return 0, _skipped_payload(summary)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"status={payload['status']} missing_sections={len(payload['missing_sections'])} "
        f"missing_done_tasks={len(payload['missing_done_task_refs'])} out={_norm(out)}"
    )
