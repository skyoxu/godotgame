...
SKILLS = {
    ...
    "workflow-chapter3-task-triplet-baseline": {
        ...
        "desc": "Run the fixed Chapter 3 task triplet baseline workflow from workflow.md. Use when a business repo is initialized with authoritative Taskmaster triplet files, when new tasks are added, when tasks.json must be rebuilt from tasks_back.json and tasks_gameplay.json, or when task links, refs, triplet consistency, or semantic review tier baseline must be validated before Chapter 4 overlays.",
        "chapter": "3",
        "purpose": "create or refresh the authoritative task triplet baseline before overlays, contracts, and Chapter 6 execution depend on it",
        "default": "For new project initialization, build or confirm all three task files and run the full baseline. For added tasks, rerun the baseline and semantic review tier validation before Chapter 4 or Chapter 6 uses the changed task set.",
        "command": "py -3 scripts/python/task_links_validate.py && py -3 scripts/python/check_tasks_all_refs.py && py -3 scripts/python/validate_task_master_triplet.py",
        "evidence": "Chapter 3 depends on real triplet files. This template repo may not have them, so use business-repo triplet structure as evidence but always validate the target repo directly.",
        "steps": [
            ...
            "Confirm `.taskmaster/tasks/tasks.json`, `tasks_back.json`, and `tasks_gameplay.json` exist or build `tasks.json` from the two view files when appropriate.",
...
def workflow_chapter_summary(template: Path, chapter: str) -> str:
    ...
    heading_aliases = {
        ...
        "3.2": "3.2 Build authoritative triplet",
        "3.3": "3.3 Validate triplet baseline",
        "3.4": "3.4 Standardize semantic review tier early",
