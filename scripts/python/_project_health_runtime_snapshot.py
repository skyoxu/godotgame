"""Immutable runtime input preparation for Project Health verification."""
...
def _is_regenerable_gdunit_import(path: str) -> bool:
    ...
    return (
        path.startswith("Tests.Godot/addons/gdUnit4/")
...
def prepare_snapshot(root: Path, destination: Path, revision: str, mode: str, deadline: float) -> dict:
    ...
    if mode == "main":
        ...
        proc = subprocess.run(
            ["git", "-C", str(root), "archive", "--format=zip", "-o", str(archive), revision],
            capture_output=True,
            check=False,
            timeout=remaining(),
        )
        if proc.returncode != 0:
            raise ValueError(proc.stderr.decode("utf-8", errors="replace") or "git archive failed")
        ...
        if proc.returncode != 0:
            raise ValueError(proc.stderr.decode("utf-8", errors="replace") or "git ls-files failed")
        names = proc.stdout.decode("utf-8").split("\0")
        for name in sorted(set(names) - {""}):
            remaining()
            if any(part in {".git", "logs", ".godot", "bin", "obj", "reports", "__pycache__"} for part in PurePosixPath(name).parts):
                continue
    ...
    manifest = {
        "mode": mode,
        "source_revision": revision if mode == "main" else "workspace:" + digest,
        "files": hashes,
