...
from _knowledge_catalog_builder import canonical_bytes, prefixed_sha256
...
def _evaluate(catalog: dict[str, Any], projections: dict[str, Any], policies: dict[str, Any], suite: dict[str, Any]) -> dict[str, Any]:
    ...
    for case in cases:
        consumer = case.get("consumer"); policy = policy_by.get(consumer); projection = projection_by.get(consumer)
        if not policy or not projection:
            failures.append({"id": case.get("id"), "reason": "consumer_policy_or_projection_missing"}); continue
        request = {"query": case.get("query")}
        result = locate(request, catalog, policy, set(projection.get("eligible_module_ids", [])), int(policy.get("max_candidates", 12)))
...
def publish(root: Path, authority_ref: str = "refs/heads/main") -> dict[str, Any]:
    policies = _load(root / POLICY_PATH); exclusions = _load(root / EXCLUSIONS_PATH); suite = _load(root / SUITE_PATH)
...
def check_current(root: Path) -> dict[str, Any]:
    ...
    if not pointer_path.is_file(): raise PublicationBlocked("current_pointer_missing")
    pointer = _load(pointer_path); manifest, artifacts = _validate_generation(root, pointer, True)
    for name in ("snapshot", "catalog", "projections"):
        if _load(root / CANONICAL[name]) != artifacts[name]: raise PublicationBlocked(f"canonical_artifact_mismatch:{name}")
    if _load(root / POLICY_PATH) != artifacts["policies"]: raise PublicationBlocked("current_policy_mismatch")
    if _load(root / EXCLUSIONS_PATH) != artifacts["exclusions"]: raise PublicationBlocked("current_exclusions_mismatch")
    if _load(root / SUITE_PATH) != artifacts["query_suite"]: raise PublicationBlocked("current_query_suite_mismatch")
    if artifacts["evaluation"].get("status") != "passed": raise PublicationBlocked("published_evaluation_not_passed")
...
def restore_lkg(root: Path) -> dict[str, Any]:
    ...
    if not path.is_file(): raise PublicationBlocked("lkg_pointer_missing")
    pointer = _load(path); _, artifacts = _validate_generation(root, pointer, False)
    for name in ("snapshot", "catalog", "projections"):
        write_json(root / CANONICAL[name], artifacts[name])
    write_json(root / INDEX_ROOT / "current.json", pointer)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--repository-root", type=Path, default=Path.cwd()); parser.add_argument("--authority-ref", default="refs/heads/main")
    mode = parser.add_mutually_exclusive_group(required=True); mode.add_argument("--publish", action="store_true"); mode.add_argument("--check", action="store_true"); mode.add_argument("--restore-lkg", action="store_true")
    args = parser.parse_args(argv); root = args.repository_root.resolve()
    try:
        ...
        if exc.details: payload["details"] = exc.details
        print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True)); return 2
    print(json.dumps({"schema": "godot-project-knowledge.publication-result.v1", **result}, ensure_ascii=False, indent=2, sort_keys=True)); return 0
