...
class Chapter7UiWiringTests(unittest.TestCase):
    ...
    def test_collect_should_skip_when_real_task_triplet_is_missing(self) -> None:
        module = _load_module("collect_ui_wiring_inputs_missing_module", "scripts/python/collect_ui_wiring_inputs.py")
        with tempfile.TemporaryDirectory() as td:
            summary = module.build_summary(repo_root=Path(td))
        self.assertEqual("skipped", summary["status"])
        self.assertEqual("missing_task_triplet", summary["reason"])
    ...
    def test_validate_should_skip_when_task_triplet_is_missing(self) -> None:
        validator = _load_module("validate_chapter7_ui_wiring_skip_module", "scripts/python/validate_chapter7_ui_wiring.py")
        with tempfile.TemporaryDirectory() as td:
            rc, payload = validator.validate(repo_root=Path(td))
