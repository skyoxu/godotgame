...
TMP_ROOT = REPO_ROOT / "tmp" / "pytest-chapter7"
...
class Chapter7UiWiringTests(unittest.TestCase):
    @contextmanager
    def _tmpdir(self):
        root = TMP_ROOT / f"case-{uuid.uuid4().hex}"
        if root.exists():
    ...
    def test_orchestrator_should_skip_safely_when_task_triplet_is_missing_in_template_repo(self) -> None:
        run_module = _load_module("run_chapter7_ui_wiring_module_for_missing_triplet", "scripts/python/run_chapter7_ui_wiring.py")
        with self._tmpdir() as td:
            root = Path(td)
            out_json = root / "logs" / "ci" / "summary.json"
            rc = run_module.main([
        ...
        self.assertEqual(0, rc)
        self.assertEqual("ok", payload["status"])
        self.assertEqual("collect", payload["steps"][0]["name"])
        self.assertEqual(0, payload["steps"][0]["rc"])
        self.assertEqual("artifact-manifest", payload["steps"][-1]["name"])
