...
def read_test_evidence(directory: Path, kind: str, expected: str, min_tests: int) -> dict:
    ...
    try:
        for path in files:
            ...
            expected_tag = "UnitTestResult" if kind == "dotnet" else "testcase"
            local_cases = [node for node in tree.iter() if node.tag.split("}")[-1] == expected_tag]
            identities = [
                case.get("executionId")
                or case.get("testName")
                or (case.get("classname", "") + ":" + case.get("name", ""))
            ...
            for node in tree.iter():
                ...
                if tag in {"testsuite", "testsuites"} and "tests" in node.attrib:
                    if int(node.get("tests")) != len(
                        [case for case in node.iter() if case.tag.split("}")[-1] == "testcase"]
                    ):
                ...
                if tag == "Counters":
                    ...
                    if any(
                        key in node.attrib and int(node.get(key)) != count
            ...
            for node in tree.iter():
                tag = node.tag.split("}")[-1]
                if tag in {"testsuites", "testsuite"} and any(
                    int(node.get(key, "0")) > 0 for key in ("errors", "failures", "skipped")
                ):
            ...
            if kind == "dotnet":
                ...
                for suite in [n for n in tree.iter() if n.tag.split("}")[-1] == "testsuite"]:
                    for case in [n for n in suite if n.tag.split("}")[-1] == "testcase"]:
                        if suite.get("name", "") != expected or case.get("classname", expected) != expected:
                            return {**result, "reason": "unexpected-test-selection"}
                        result["tests"] += 1
                        result["failed"] += any(
                            child.tag.split("}")[-1] in {"failure", "error"} for child in case
                        )
                        for failure in [
                            child for child in case if child.tag.split("}")[-1] == "failure"
                        ]:
                            result["failure_messages"].append("".join(failure.itertext()))
                        result["skipped"] += any(
                            child.tag.split("}")[-1] == "skipped" for child in case
                        )
        ...
        result["reason"] = (
            "verified"
            if result["passed"]
            else "failed-skipped-or-empty-tests"
    ...
    *,
    ...
    if test["kind"] == "dotnet":
        ...
        command = [
            ...
            "Tests.Godot",
            "--add",
            test["path"].removeprefix("Tests.Godot/"),
            "--timeout-sec",
    ...
    evidence = read_test_evidence(evidence_root, test["kind"], expected, test["min_tests"])
    return {
        "id": test["id"],
        "evidence_level": test["evidence_level"],
        "command": command,
        "exit_code": rc,
        "status": "passed" if rc == 0 and evidence["passed"] else "failed",
        "evidence": evidence,
        "log": str(out / "console.log"),
    }
