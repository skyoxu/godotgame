...
def orchestrate(*, repo_root: Path, delivery_profile: str, write_doc: bool) -> tuple[int, dict[str, Any]]:
    ...
    if overall_rc == 0:
        ...
        if input_source.exists():
            ...
            if source_payload.get('status') == 'skipped':
                payload['status'] = 'ok'
                payload['skip_reason'] = source_payload.get('reason') or 'missing_task_triplet'
                payload['missing_source_files'] = source_payload.get('missing_source_files', [])
