...
def _semantic_prompt_entries(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for entry in entries:
        ...
        if kind not in {"config", "asset", "scene"}:
            continue
        item: dict[str, Any] = {key: entry.get(key) for key in ("id", "path", "kind", "role", "task_title", "readers", "evidence", "test_refs")}
        if kind == "config":
            fields = entry.get("parameters") if isinstance(entry.get("parameters"), list) else []
            prioritized = sorted((field for field in fields if isinstance(field, dict) and isinstance(field.get("pointer"), str)), key=lambda field: (field.get("kind") != "numeric_parameter", field.get("line") or 0))
            item["available_parameters"] = [{key: field.get(key) for key in ("pointer", "value", "line", "record_id", "kind")} for field in prioritized[:80]]
            item["available_parameters_truncated"] = len(fields) > 80
        elif kind == "asset":
            bindings = entry.get("bindings") if isinstance(entry.get("bindings"), list) else []
            item["available_bindings"] = [{key: binding.get(key) for key in ("source", "line", "evidence")} for binding in bindings[:12] if isinstance(binding, dict)]
            item["available_bindings_truncated"] = len(bindings) > 12
        else:
            bindings = entry.get("bindings") if isinstance(entry.get("bindings"), list) else []
            item["available_bindings"] = [{"node_path": binding.get("node_path"), "type": binding.get("type"), "line": binding.get("line"), "properties": [{key: prop.get(key) for key in ("name", "value", "line")} for prop in binding.get("properties", [])[:12] if isinstance(prop, dict)]} for binding in bindings[:20] if isinstance(binding, dict)]
...
def _validate_semantic_entries(model: object, entries: list[dict[str, Any]]) -> tuple[bool, list[dict[str, Any]], str | None]:
    ...
    for item in model:
        ...
        fields_by_pointer = {str(field.get("pointer")): field for field in fields if isinstance(field, dict) and isinstance(field.get("pointer"), str)}
        ...
        for parameter in parameters:
            ...
            validated_parameters.append({"pointer": pointer, "meaning": str(parameter.get("meaning") or parameter.get("description") or ""), "value": field.get("value"), "line": field.get("line"), "evidence_status": "field_exists_semantic_inference"})
        ...
        if kind == "asset":
            available_bindings = {(binding.get("source"), binding.get("line")): binding for binding in source_bindings if isinstance(binding, dict)}
            for binding in bindings:
                ...
                validated_bindings.append({"source": actual.get("source"), "line": actual.get("line"), "evidence": actual.get("evidence"), "meaning": str(binding.get("meaning") or ""), "evidence_status": "static_binding_semantic_explanation"})
        elif kind == "scene":
            available_bindings = {(binding.get("node_path"), binding.get("line")): binding for binding in source_bindings if isinstance(binding, dict)}
            for binding in bindings:
                key = (binding.get("node_path"), binding.get("line"))
                if key not in available_bindings or not str(binding.get("meaning") or "").strip():
                    return False, [], f"unknown or unexplained scene binding for {path}: {key}"
                actual = available_bindings[key]
                validated_bindings.append({"node_path": actual.get("node_path"), "type": actual.get("type"), "line": actual.get("line"), "meaning": str(binding.get("meaning") or ""), "evidence_status": "static_binding_semantic_explanation"})
        elif bindings:
            return False, [], f"bindings are not supported for {path}"

        normalized.append({"id": source.get("id"), "path": path, "kind": kind, "explanation": str(item.get("explanation") or ""), "modification_guidance": str(item.get("modification_guidance") or item.get("parameter_guidance") or ""), "modification_impact": str(item.get("modification_impact") or ""), "parameters": validated_parameters, "bindings": validated_bindings})
...
def _semantic_enrich(root: Path, task_id: str, backend: str) -> dict[str, Any]:
    ...
    entries = [item for item in payload.get("generated", []) if isinstance(item, dict) and str(item.get("task_id")) == str(task_id)]
    ...
    for attempt in range(1, 6):
        rc, trace, command = run_llm_exec(backend=backend, root=root, prompt=prompt, output_last_message=output, timeout_sec=300)
        if rc == 0:
            try:
    ...
    if rc != 0 or model is None:
        write_json(output, {"status": "unverified", "task_id": str(task_id), "generated_by": "llm-assisted-semantic-analysis", "attempts": 5, "error": validation_error or trace[-1000:] or "semantic output validation failed"})
        return {"status": "unverified", "rc": rc, "attempts": 5}
    write_json(output, {"status": "verified", "task_id": str(task_id), "generated_by": "llm-assisted-semantic-analysis", "backend": backend, "entries": model})
    return {"status": "verified", "entries": len(model), "attempts": attempt, "command": command}


def run(root: Path, task_id: str, paths: list[str] | None = None, *, write_task_refs: bool = False, semantic: bool = False, llm_backend: str = "codex-cli") -> dict[str, Any]:
    root = root.resolve()
    initialize(root)
    scanned = scan(root)
    links = generate(root, {str(task_id)}, write_task_refs)
    reviewed = capture(root, task_id, paths or [])
    element_capture = capture_element_manifest(root, str(task_id), str(scanned.get("revision") or ""))
    result: dict[str, Any] = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run(args.repo_root.resolve(), str(args.task_id), args.path, write_task_refs=args.write_task_refs, semantic=args.semantic, llm_backend=args.llm_backend)
