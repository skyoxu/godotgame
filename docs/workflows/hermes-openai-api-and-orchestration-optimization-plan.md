...
- Step 6 partial: `scripts/python/run_single_task_light_lane.py` and `scripts/python/run_single_task_light_lane_batch.py` now emit Chapter 6-style route fields (`preferred_lane`, `recommended_action`, `recommended_command`, `forbidden_commands`, `latest_reason`, `recommended_action_why`) in both self-check and real summaries. They now also classify timeout backoff, retry-soft-steps-only, inspect-first preflight gaps, extract-only reruns, and split-batch stop-loss cases into stable route-contract outputs, while batch recommendations use batch-aware rerun commands instead of implying a blind full rerun.
- Step 7 partial: `scripts/sc/_llm_backend.py` now exists as the internal provider seam for `llm_review`; backend dispatch is no longer hard-coded inside `_llm_review_exec.py`, `llm_review` self-check / dry-run expose backend readiness (`codex-cli|openai-api`), `openai-api` now has a minimal runnable Responses API path for `llm_review`, and the higher-level entrypoints `scripts/sc/run_review_pipeline.py` plus `scripts/sc/llm_review_needs_fix_fast.py` now both pass through `--llm-backend` so the transport can be piloted without bypassing the normal orchestration layer. The Batch 2 semantics pilots now cover five families through the same seam: `scripts/sc/llm_extract_task_obligations.py` routes consensus rounds through the shared backend seam, `scripts/sc/llm_align_acceptance_semantics.py` forwards `--llm-backend` into acceptance-semantics alignment retries, `scripts/sc/llm_fill_acceptance_refs.py` now accepts `--llm-backend` while routing its consensus mapping calls through the same shared transport layer, `scripts/sc/llm_check_subtasks_coverage.py` now resolves `--llm-backend` through the same seam for its per-round semantic coverage calls, and `scripts/sc/llm_semantic_gate_all.py` now routes batch semantic gate runs through the same backend layer while preserving `model_reasoning_effort`. Batch 3 has also started: `scripts/sc/llm_generate_tests_from_acceptance_refs.py` now accepts `--llm-backend`, and both primary-ref selection plus per-file test generation calls route through the same shared backend seam. `scripts/sc/llm_generate_red_test.py` now also accepts `--llm-backend`, so the narrow red-test drafting helper follows the same transport contract. Default behavior still stays on `codex-cli`.
