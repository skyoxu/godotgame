...
- `Project Health local-server unittest coverage`（其余 template Project Health core regressions 由 Windows Quality Gate 的 `scripts/ci/tests` discover 步骤硬阻断）
- `cross-repo migration reconciliation unittest coverage`（manifest 完整性、copy_exact 漂移、source inventory 与 canonical PR guard；随 `obligations_unittest` 一并阻断）
