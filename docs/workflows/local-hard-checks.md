...
Notes:
- For test-oriented commands, `--solution auto` prefers the test-bearing solution when one exists (for this repo, that is currently `Game.sln` rather than a repo-named solution file).
