extends "res://addons/gdUnit4/src/GdUnitTestSuite.gd"

func _new_db(name: String) -> Node:
    var db = preload("res://Game.Godot/Adapters/SqliteDataStore.cs").new()
    db.name = name
    get_tree().get_root().add_child(auto_free(db))
    return db

func test_savegame_unique_slot_per_user() -> void:
    var path = "user://utdb_%s/save_unique.db" % Time.get_unix_time_from_system()
    var db = _new_db("SqlDb")
    var ok = db.TryOpen(path)
    assert_bool(ok).is_true()

    var helper = preload("res://Game.Godot/Adapters/Db/DbTestHelper.cs").new()
    add_child(auto_free(helper))
    helper.CreateSchema()
    helper.ClearAll()

    var bridge = preload("res://Game.Godot/Adapters/Db/RepositoryTestBridge.cs").new()
    add_child(auto_free(bridge))
    var username = "u_%s" % Time.get_unix_time_from_system()
    assert_bool(bridge.UpsertUser(username)).is_true()
    var uid = bridge.FindUserId(username)
    assert_that(uid).is_not_null()

    # upsert twice on same slot, should result in exactly one row
    var json1 = '{"hp": 10, "ts": %d}' % Time.get_unix_time_from_system()
    assert_bool(bridge.UpsertSave(uid, 1, json1)).is_true()
    var json2 = '{"hp": 77, "ts": %d}' % Time.get_unix_time_from_system()
    assert_bool(bridge.UpsertSave(uid, 1, json2)).is_true()

    # validate via GetSaveData
    var got = bridge.GetSaveData(uid, 1)
    assert_str(str(got)).contains('"hp": 77')

    # and ensure ListByUser returns exactly one entry for slot 1
    var list = bridge.ListSaveJsonByUser(uid)
    assert_int(list.size()).is_greater_equal(1)
    var count_slot1 := 0
    for s in list:
        if str(s).find('"hp": 77') != -1:
            count_slot1 += 1
    assert_int(count_slot1).is_equal(1)

